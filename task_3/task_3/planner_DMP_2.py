...
class Planner_DMP(Node):
    ...
    def _plan_with_dmp(self, marker_id, pose):
        ...
        T_start = self.robot.fkine(q0)  # matrice 4x4 (SE3)
        T_end = self.robot.fkine(qf) # matrice 4x4 (SE3)
        T_end_numpy = self.robot.fkine(qf).A # matrice 4x4 (numpy)
        T_start_numpy = self.robot.fkine(q0).A # matrice 4x4 (numpy)

        q_traj = np.array([[0.35, *q[1:]] for q in q_traj_sampled])  # shape (N, 8)

        # --- Calcola la cinematica diretta per ogni configurazione
        trajectory_from_dataset = [self.robot.fkine(q).A for q in q_traj]  # lista di 4x4 SE3 in numpy
        ...
        # Filtro: tieni solo pose con rotazione ben definita
        def is_valid_rotation(R_mat):
            should_be_identity = R_mat.T @ R_mat
            I = np.eye(3)
            error = np.linalg.norm(I - should_be_identity)
            det = np.linalg.det(R_mat)
            return error < 1e-3 and abs(det - 1.0) < 1e-3

        filtered_indices = [i for i, T in enumerate(trajectory_reduced) if is_valid_rotation(T[:3, :3])]
        trajectory_filtered = [trajectory_reduced[i] for i in filtered_indices]

        self.get_logger().info(f"Pose totali: {len(trajectory_generalized)}")
        self.get_logger().info(f"Pose scartate: {len(trajectory_reduced) - len(trajectory_filtered)}")

        joint_trajectory = []
        q_curr = q0
        c=0
        for T_d in trajectory_filtered:
            T_se3 = SE3(T_d)
        ...
        positions_new = np.array([T[:3, 3] for T in trajectory_filtered])
